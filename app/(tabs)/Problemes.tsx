...
export default function Probleme() {
    ...
    const renderTestItem = ({ item }: any) => (
        <TouchableOpacity
            ...
            onPress={() => navigation.navigate("TemplateTest", { idTest: item.id, testName: item.title })}
        >
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.filterContainer}>
                <TouchableOpacity
                    style={[styles.filterButton, selectedStudy === "" && styles.activeFilterButton]}
                    onPress={() => handleFilterChange("")}
                >
                    <Text style={styles.filterButtonText}>Tous</Text>
                </TouchableOpacity>
                <TouchableOpacity
                    style={[styles.filterButton, selectedStudy === "MMI" && styles.activeFilterButton]}
                    onPress={() => handleFilterChange("MMI")}
                >
                    <Text style={styles.filterButtonText}>MMI</Text>
                </TouchableOpacity>
                <TouchableOpacity
                    style={[styles.filterButton, selectedStudy === "GEA" && styles.activeFilterButton]}
                    onPress={() => handleFilterChange("GEA")}
                >
                    <Text style={styles.filterButtonText}>GEA</Text>
                </TouchableOpacity>
                <TouchableOpacity
                    style={[styles.filterButton, selectedStudy === "TC" && styles.activeFilterButton]}
            ...
            {tests.length === 0 ? (
                <View style={styles.noResultsContainer}>
                    <Text style={styles.noResultsText}>Aucun test trouvé pour la filière sélectionnée.</Text>
                </View>
            ) : (
                <FlatList
                    data={tests}
...
const styles = StyleSheet.create({
    ...
    testList: {
        ...
        flexWrap: "wrap",
        justifyContent: "center",
        gap: 20, // Utilisez `margin` sur les enfants si `gap` n'est pas pris en charge
        padding: 10,
    },
    testItem: {
        flexBasis: "30%", // Largeur de 30% pour chaque carte
        marginBottom: 20,
        padding: 20,
        borderWidth: 1,
        borderColor: "#ddd",
    ...
    testItemHover: {
        backgroundColor: "#f1f1f1", // Simulez un hover avec un état dans React Native
    },
    testTitle: {
        fontSize: 20,
        color: "#432683",
